...
<head>
  ...
  <style>
    ...
    * {{ box-sizing:border-box; }}
    ...
    .comments-entry__previews {{ display:grid; gap:0; border-top:1px solid var(--line); border-bottom:1px solid var(--line); }}
    .comments-entry__preview {{ display:grid; grid-template-columns:auto minmax(0,1fr); gap:8px; padding:7px 0; border-bottom:1px solid var(--line); font-family:var(--sans); }}
    .comments-entry__preview:last-child {{ border-bottom:0; }}
    ...
    .comments-compact__empty {{ margin:0; padding:11px 2px; color:var(--muted); font:400 11px/1.5 var(--sans); }}
    .comment-mini {{ padding:9px 2px; border-bottom:1px solid var(--line); }} .comment-mini:last-child {{ border-bottom:0; }}
    .comment-mini__head {{ display:flex; justify-content:space-between; gap:10px; margin-bottom:4px; font-family:var(--sans); }}
    .comment-mini__head strong {{ font-size:11px; }} .comment-mini__head time {{ color:var(--muted); font-size:9px; }}
    .comment-mini p {{ margin:0; color:var(--ink); font-size:12px; line-height:1.55; white-space:pre-wrap; overflow-wrap:anywhere; }}
    ...
    @media (max-width:420px) {{
      .comments-sheet__panel {{ min-height:340px; }}
...
<body>
    ...
    <section class="hero" aria-labelledby="recordTitle">
      ...
      <aside class="hero-side" aria-label="专题封面与评论">
        ...
        <button class="comments-entry" id="commentsEntry" type="button" aria-haspopup="dialog">
          <span class="comments-entry__head"><span class="comments-entry__title">读者片言</span><span class="comments-entry__count" id="commentsEntryCount">0 条评论</span></span>
  ...
  <section class="comments-sheet" id="commentsSheet" hidden role="dialog" aria-modal="true" aria-labelledby="commentsSheetTitle">
    <div class="comments-sheet__panel">
      <div class="comments-sheet__inner">
        ...
        <section class="comments-compact" aria-labelledby="localCommentsTitle">
          <header class="comments-compact__head"><h3 id="localCommentsTitle">读者片言</h3><span class="comments-compact__count" id="localCommentCount">0 条评论</span></header>
          <div class="comments-compact__body">
            <div class="comments-compact__list" id="localCommentList"><p class="comments-compact__empty">暂无评论</p></div>
            <form class="comments-compact__form" id="localCommentForm">
              <div class="comments-compact__fields">
                ...
                <label>邮箱（选填）<input name="email" type="email" maxlength="160" autocomplete="email" placeholder="不会公开"></label>
              </div>
              <label>内容<textarea name="body" maxlength="1200" required placeholder="关于这个词、这段历史，或你自己的世界……"></textarea></label>
              <input class="comments-compact__trap" name="website" tabindex="-1" autocomplete="off" aria-hidden="true">
              <div class="comments-compact__actions"><p class="comments-compact__status" id="localCommentStatus" aria-live="polite">本地模拟，不会上传。</p><button type="submit">写入此页</button></div>
            </form>
          </div>
        </section>
      </div>
  ...
  <script>
    (() => {{
      ...
      const commentStoreKey = 'crivu-world-record-comments-preview';
      const commentForm = document.querySelector('#localCommentForm');
      const commentList = document.querySelector('#localCommentList');
      const commentCount = document.querySelector('#localCommentCount');
      ...
      const commentStatus = document.querySelector('#localCommentStatus');
      ...
      let localComments = [];
      try {{
        const stored = JSON.parse(localStorage.getItem(commentStoreKey) || '[]');
        if (Array.isArray(stored)) localComments = stored.filter((item) => item && typeof item.authorName === 'string' && typeof item.body === 'string').slice(-20);
      }} catch {{}}
      const renderComments = () => {{
        commentList.replaceChildren();
        commentsEntryPreviews.replaceChildren();
        commentCount.textContent = `${{localComments.length}} 条评论`;
        commentsEntryCount.textContent = `${{localComments.length}} 条评论`;
        if (!localComments.length) {{
          const empty = document.createElement('p');
          empty.className = 'comments-compact__empty';
        ...
        [...localComments].reverse().slice(0,2).forEach((item) => {{
        ...
        [...localComments].reverse().forEach((item) => {{
          const article = document.createElement('article');
          article.className = 'comment-mini';
          const head = document.createElement('div');
          head.className = 'comment-mini__head';
          const name = document.createElement('strong');
          name.textContent = item.authorName;
          const time = document.createElement('time');
          const date = new Date(item.createdAt);
          time.dateTime = Number.isNaN(date.getTime()) ? '' : date.toISOString();
          time.textContent = Number.isNaN(date.getTime()) ? '刚刚' : new Intl.DateTimeFormat('zh-CN', {{ year:'numeric', month:'2-digit', day:'2-digit' }}).format(date);
          const bodyText = document.createElement('p');
          bodyText.textContent = item.body;
          head.append(name, time);
          article.append(head, bodyText);
      ...
      commentForm.addEventListener('submit', (event) => {{
        ...
        if (!authorName || !bodyText) {{ commentStatus.textContent = '请填写昵称与评论内容。'; return; }}
        localComments = [...localComments, {{ authorName: authorName.slice(0,32), body: bodyText.slice(0,1200), createdAt:new Date().toISOString() }}].slice(-20);
        try {{ localStorage.setItem(commentStoreKey, JSON.stringify(localComments)); }} catch {{}}
        commentForm.reset();
        renderComments();
        commentStatus.textContent = '已写入本地，仅保存在此浏览器。';
      }});
      renderComments();
